...
impl<L, T, E> ParseError<L, T, E> {
    ...
    ///
    /// This could be useful for example to obtain a `ParseError` where all
    /// fields implement `Display` or `Debug`.  `ParseErrors` where all fields
    /// implement `Display` also implement `Display` and `ParseErrors` where all
    /// fields implement both `Display` and `Debug` implement `Error`
