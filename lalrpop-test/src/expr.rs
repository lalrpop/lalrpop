    ...
    pub fn __state7<TOKENS: Iterator<Item=Tok>>(
        ...
        match lookahead {
            Some(__tok @ Tok::Times(..)) => {
                let mut sym1 = &mut Some(__tok);
                let lookahead = tokens.next();
                result = try!(__state16(lookahead, tokens, sym0, sym1));
            }
            Some(__tok @ Tok::Div(..)) => {
                let mut sym1 = &mut Some(__tok);
                let lookahead = tokens.next();
                result = try!(__state15(lookahead, tokens, sym0, sym1));
            }
            Some(Tok::Minus(..)) => {
                let sym0 = sym0.take().unwrap();
                let nt = super::__action3(sym0);
    ...
    pub fn __state27<TOKENS: Iterator<Item=Tok>>(
        ...
        match lookahead {
            Some(__tok @ Tok::Times(..)) => {
                let mut sym3 = &mut Some(__tok);
                let lookahead = tokens.next();
                result = try!(__state16(lookahead, tokens, sym2, sym3));
            }
            Some(__tok @ Tok::Div(..)) => {
                let mut sym3 = &mut Some(__tok);
                let lookahead = tokens.next();
                result = try!(__state15(lookahead, tokens, sym2, sym3));
            }
            Some(Tok::Minus(..)) => {
                let sym0 = sym0.take().unwrap();
                let sym1 = sym1.take().unwrap();
