...
mod __parse__Expr {
    ...
    //   Num = r#"[0-9]+"# (*) ["/"]
    //
    //   EOF -> Reduce(Num = r#"[0-9]+"# => Call(ActionFn(11));)
    //   "*" -> Reduce(Num = r#"[0-9]+"# => Call(ActionFn(11));)
    //   "-" -> Reduce(Num = r#"[0-9]+"# => Call(ActionFn(11));)
    //   "/" -> Reduce(Num = r#"[0-9]+"# => Call(ActionFn(11));)
    //   "+" -> Reduce(Num = r#"[0-9]+"# => Call(ActionFn(11));)
    //
    ...
    //   Term = Num (*) ["/"]
    //
    //   "+" -> Reduce(Term = Num => Call(ActionFn(9));)
    //   "/" -> Reduce(Term = Num => Call(ActionFn(9));)
    //   "-" -> Reduce(Term = Num => Call(ActionFn(9));)
    //   EOF -> Reduce(Term = Num => Call(ActionFn(9));)
    //   "*" -> Reduce(Term = Num => Call(ActionFn(9));)
    //
    pub fn __state5<
        'input,
    ...
    //
    //   "*" -> Shift(S16)
    //   EOF -> Reduce(Expr = Factor => Call(ActionFn(2));)
    //   "/" -> Shift(S18)
    //   "-" -> Reduce(Expr = Factor => Call(ActionFn(2));)
    //   "+" -> Reduce(Expr = Factor => Call(ActionFn(2));)
    //
    //   FactorOp -> S17
    pub fn __state6<
        ...
        while __sym1.is_some() {
            ...
            match __nt {
                __Nonterminal::ExprOp(__nt) => {
                    ...
                    __result = try!(__state22(input, __lookbehind, __tokens, __lookahead, __sym1, __sym2));
                }
                _ => {
                    return Ok((__lookbehind, __lookahead, __nt));
                }
    ...
    ) -> Result<(Option<usize>, Option<(usize, (usize, &'input str), usize)>, __Nonterminal<>), __ParseError<usize,(usize, &'input str),()>>
    {
        ...
        match __lookahead {
            ...
            Some((_, (5, __tok0), __loc)) => {
                let mut __lookbehind = Some(__loc);
                let mut __sym2 = &mut Some((__tok0));
                __result = try!(__state1(input, __lookbehind, __tokens, __sym2));
            }
            _ => {
                return Err(__ParseError::UnrecognizedToken {
                    token: __lookahead,
    ...
    //   ExprOp = "+" (*) [r#"[0-9]+"#]
    //
    //   r#"[0-9]+"# -> Reduce(ExprOp = "+" => Call(ActionFn(3));)
    //   "(" -> Reduce(ExprOp = "+" => Call(ActionFn(3));)
    //
    pub fn __state14<
        'input,
        __TOKENS: Iterator<Item=Result<(usize, (usize, &'input str), usize),__ParseError<usize,(usize, &'input str),()>>>,
    ...
    //   Term = "(" Expr ")" (*) ["/"]
    //
    //   "/" -> Reduce(Term = "(", Expr, ")" => Call(ActionFn(10));)
    //   EOF -> Reduce(Term = "(", Expr, ")" => Call(ActionFn(10));)
    //   "-" -> Reduce(Term = "(", Expr, ")" => Call(ActionFn(10));)
    //   "*" -> Reduce(Term = "(", Expr, ")" => Call(ActionFn(10));)
    //   "+" -> Reduce(Term = "(", Expr, ")" => Call(ActionFn(10));)
    //
    ...
    //   FactorOp = (*) "/" [r#"[0-9]+"#]
    //
    //   "+" -> Reduce(Expr = Expr, ExprOp, Factor => Call(ActionFn(1));)
    //   "*" -> Shift(S16)
    //   "-" -> Reduce(Expr = Expr, ExprOp, Factor => Call(ActionFn(1));)
    //   "/" -> Shift(S18)
    //   EOF -> Reduce(Expr = Expr, ExprOp, Factor => Call(ActionFn(1));)
    //
    //   FactorOp -> S17
    pub fn __state23<
        'input,
        ...
        while __sym2.is_some() {
            ...
            match __nt {
                __Nonterminal::FactorOp(__nt) => {
                    ...
                    __result = try!(__state17(input, __lookbehind, __tokens, __lookahead, __sym2, __sym3));
                }
                _ => {
                    return Ok((__lookbehind, __lookahead, __nt));
                }
    ...
    //   Factor = Factor FactorOp Term (*) ["/"]
    //
    //   "-" -> Reduce(Factor = Factor, FactorOp, Term => Call(ActionFn(5));)
    //   EOF -> Reduce(Factor = Factor, FactorOp, Term => Call(ActionFn(5));)
    //   "*" -> Reduce(Factor = Factor, FactorOp, Term => Call(ActionFn(5));)
    //   "+" -> Reduce(Factor = Factor, FactorOp, Term => Call(ActionFn(5));)
    //   "/" -> Reduce(Factor = Factor, FactorOp, Term => Call(ActionFn(5));)
    //
